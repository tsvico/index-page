...
$(document).ready(function () {
  ...
  //获取搜索引擎列表
  function getSeList() {
    var se_list_local = storage.getItem('se_list');
  ...
  function setSeList(se_list) {
    if (se_list) {
      storage.setItem('se_list', JSON.stringify(se_list));
      return true;
    }
    return false;
  }
  ...
  $('.se_list_table').on('click', '.set_se_default', function () {
    var name = $(this).val();
    storage.setItem('se_default', name);
    setSeInit();
  });

  //获得默认搜索引擎
  function getSeDefault() {
    var se_default = storage.getItem('se_default');
  ...
  function setSeInit() {
    ...
    for (var i in se_list) {
      ...
      tr +=
        ...
        '. ' +
        se_list[i]['title'] +
        "</td><td><button class='set_se_default' value='" +
        i +
        "'><span class='iconfont iconstrore-add'></span></button><button class='edit_se' value='" +
        i +
        "'><span class='iconfont iconbook-edit'></span></button> <button class='delete_se' value='" +
        i +
        "'><span class='iconfont icondelete'></span></button></td></tr>";
      html += tr;
    }
  ...
  function getQuickList() {
    var quick_list_local = storage.getItem('quick_list');
  ...
  function setQuickList(quick_list) {
    if (quick_list) {
      storage.setItem('quick_list', JSON.stringify(quick_list));
      return true;
    }
    return false;
  }
  ...
  $('#my_data_in').click(function () {
    ...
    if (confirm('Current settings will be erased, continue?')) {
      ...
      if (mydata['se_default']) {
        storage.setItem('se_default', mydata['se_default']);
      }
